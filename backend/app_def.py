...
def data_request_page(request_page):
    ...
    sql1 = "SELECT {} FROM comments RIGHT JOIN shops ON comments.shop_id=shops.id {}GROUP BY shops.id ORDER BY ROUND(AVG(comments.stars),2) DESC LIMIT {},{};".format(
...
def data_request_shop_comment():
    ...
    head = "cmnt_id,username,content,likes,stars,user_avatar"
    sql = "SELECT {0} FROM comments WHERE shop_id={1} LIMIT {2},{3}".format(
        head, shop_id, (page - 1) * limit, limit
    )

    commentslist = []
    result = dbop.search(sql)
    columns = ["id", "username", "comment", "like", "star", "userimg"]
...
def addcomment():
    ...
    head = "shop_id,content,username,user_avatar,stars,user_openid"
    val = '{},"{}","{}","{}",{},"{}"'.format(
...
def getopenid():
    ...
    if len(data):
        ...
        sql = 'INSERT INTO users(openid,username,avatar) VALUES ("{}","微信用户{}","{}");'.format(
            openid, openid[0:6], func.image_url_avatar("Akkarin.jpg")
        )
        dbop.insert(sql)
        return {
            "openid": openid,
